...
def updateVehicleCoordinates(vehicles, speeds, dt):
    for direction in vehicles:
        for lane in vehicles[direction]:
            for vehicle in vehicles[direction][lane]:
                if direction == 'right':
                    vehicle.x += speeds[vehicle.vehicleClass] * dt
                elif direction == 'down':
                    vehicle.y += speeds[vehicle.vehicleClass] * dt
                elif direction == 'left':
                    vehicle.x -= speeds[vehicle.vehicleClass] * dt
                elif direction == 'up':
                    vehicle.y -= speeds[vehicle.vehicleClass] * dt
...
def updateSignalTimers(signals, dt):
...
def updateGraphics(screen, background, redSignal, yellowSignal, greenSignal, font, signals):
    ...
    for signal in signals:
        text = font.render(str(int(signal.signalText)), True, (0, 0, 0))
        screen.blit(text, signal.textLocation)




def adjust_speeds(vehicles, speeds, dt):
    for direction in vehicles:
        for lane in vehicles[direction]:
            for vehicle in vehicles[direction][lane]:
                if direction == 'right':
